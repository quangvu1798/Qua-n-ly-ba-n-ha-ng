...
{
    public partial class FTKKHTiemNang : Form
    {
        public FTKKHTiemNang()
        {
            InitializeComponent();
        }
        ...
        private void FTKKHTiemNang_Load(object sender, EventArgs e)
        {
            
            dgvKhachHang.DataSource = ActTK.KHTiemNang();
            khoitaoluoi();
            txtMaKH.Text = dgvKhachHang.Rows[0].Cells[0].Value.ToString();
            txtTenKH.Text = dgvKhachHang.Rows[0].Cells[1].Value.ToString();
            txtDiaChi.Text = dgvKhachHang.Rows[0].Cells[2].Value.ToString();
